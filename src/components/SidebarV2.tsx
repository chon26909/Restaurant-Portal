...
const SidebarV2 = () => {
    ...
    const Menu: FC = () => {
        ...
        const onCollapse = (index: number) => {
            ...
            setMenus(menus => {
                return menus.map((menu, i) => i === index ? { ...menu, collapsed: !menu.collapsed } : menu)
            })
        }
        ...
        return <>
            {menus.map((item: Menu, index) => {
                return <div key={index}>
                    <div className="p-5 active:bg-white active:opacity-10 select-none flex justify-between cursor-pointer" onClick={() => onCollapse(index)}>
                        <div>{item.name}</div>
                        <div>
    ...
    const SubMenu: FC<{ submenu?: Menu[], collapsed?: boolean }> = (props) => {
        ...
        return <div className={['transition-all ease-linear duration-200', isCollapsed(props.collapsed)].join(" ")} >
            {props.submenu?.map((item: any, index) => {
                return <div key={index} className="p-3 pl-7 active:bg-white active:opacity-10">{item.name}</div>
            })}
        </div>
    }
